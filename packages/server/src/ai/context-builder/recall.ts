...
export interface RecallInput {
  ...
  topK?: number;
  /**
   * 是否把最近 10 章窗也纳入打分。默认 false:写作上下文构建时,最近 10 章已分级以
   * "近章原文/小总结/中程摘要"注入,这里要排除以免重复。用户主动 /recall 检索全书时应置 true。
   */
...
export function recallChapters(input: RecallInput): ChapterSummary[] {
  ...
  const cutoff = input.includeRecent ? input.currentChapterNo : input.currentChapterNo - 10;
