...
请按需调用工具,逐项记录:
1. **题材板块条目**(add_genre_section_item):本章出现的功法、法器、丹药、势力、境界等,若板块里还没有就添加。data 字段必须符合该板块的 schema(概要里有列出)。
2. **角色状态**(update_character_state):角色的位置/修为/持有物/心境发生重要变化时更新。
3. **角色出场**(add_character_appearance):每个有实质戏份的角色记一条本章概括。
4. **伏笔**(add_foreshadowing / pay_foreshadowing):本章埋下或回收的悬念。
5. **时间线**(add_timeline_event):本章 1-3 个关键事件。

纪律:
- 只记录本章**确实出现**的内容,不要脑补未出现的设定
- 已记录过的条目不要重复添加(概要里列了已有条目)
