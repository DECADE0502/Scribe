...
export interface WorkflowStaging {
  ...
  replace(runId: string, changes: StagedChange[]): void;
...
export function createWorkflowStaging(
  ...
  return {
    begin(runId, opts): void {
      runsRepo.create(opts.bookId, opts.source, runId);
    },
    add(runId, change): void {
    ...
    replace(runId, changes): void {
      runsRepo.deleteChanges(runId);
    ...
    commit(runId, handle): CommitResult {
      ...
      for (const ch of changes) {
        ...
        const sc: StagedChange = {
          id: ch.id,
          type: ch.type as StagedChange["type"],
          payload: ch.payload,
        ...
        try {
          ...
          failed.push({ change: sc, error: (e as Error).message });
        }
      }

      if (failed.length > 0) {
        runsRepo.setPhase(runId, "failed", "fail");
      } else {
        runsRepo.setPhase(runId, "completed", "pass");
      }
...
function applyChange(handle: BookHandle, change: StagedChange): void {
  switch (change.type) {
    ...
    case "chapter_version": {
      ...
      try {
        handle.chapterFiles.save({
          chapterNo: p.chapterNo,
          content: p.content,
          title: p.title ?? `第 ${p.chapterNo} 章`,
          versionNo: saved.versionNo,
        });
      } catch (e) {
        try {
          handle.chaptersRepo.deleteVersion(p.chapterNo, saved.versionNo);
        } catch {
          // Ignore rollback cleanup failures; the original save error is more useful.
        }
        throw e;
      }
      return;
    ...
    case "chapter_audit": {
      const p = change.payload as Parameters<typeof handle.chaptersRepo.saveAudit>[0];
      handle.chaptersRepo.saveAudit(p);
      return;
    }
    case "foreshadowing_upsert": {
      ...
      if (p.id) handle.foreshadowingRepo.update(p.id, p as any);
      else handle.foreshadowingRepo.create(p as any);
      return;
    }
    case "timeline_event": {
      const p = change.payload as {
    ...
    case "outline_upsert": {
      ...
      if (p.id) handle.outlineRepo.update(p.id, p as any);
      else {
        handle.outlineRepo.create({
          ...
          sortOrder: p.sortOrder ?? change.sortOrder ?? 0,
    ...
    case "worldbook_upsert": {
      ...
      if (p.id) handle.worldbookRepo.update(p.id, p as any);
      else handle.worldbookRepo.create(p as any);
      return;
    }
    case "record_upsert": {
